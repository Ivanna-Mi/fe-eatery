...
const menuItems = [
  ...
  { icon: ChefHat, label: "Cooking Queue", href: "/kitchen/queue" },
  { icon: CheckCircle, label: "Ready Orders", href: "/kitchen/ready-orders" },
];

export function KitchenSidebar({ collapsed, setCollapsed }: { collapsed: boolean, setCollapsed: (val: boolean) => void }) {
      ...
      <div className="h-20 flex items-center justify-between px-4 border-b border-white/10 shrink-0">
        {!collapsed && (
          <div className="flex items-center gap-3">
            ...
            <span className="font-serif font-bold text-xl whitespace-nowrap">Kitchen KDS</span>
          </div>
        )}
        {collapsed && (
          ...
          return (
            <Link key={item.href} href={item.href}>
              ...
              >
                ...
                {isActive && !collapsed && (
                  <motion.div layoutId="kitchen-sidebar-active" className="absolute left-0 w-1 h-8 bg-white rounded-r-full" />
                )}
              </div>
            </Link>
          );
