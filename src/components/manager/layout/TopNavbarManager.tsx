...
export function TopNavbarManager({ onMenuClick }: { onMenuClick: () => void }) {
  const router = useRouter();
  const { logout, managerData } = useManagerStore();

  const handleLogout = () => {
    logout();
    router.push("/manager/login"); // Redirecting to manager login
  };
    ...
    <header className="h-20 bg-background/80 backdrop-blur-md border-b border-border/50 flex items-center justify-between px-6 sticky top-0 z-10">
      <div className="flex items-center gap-4">
        ...
        <div className="hidden md:flex relative items-center">
          ...
          <input 
            type="text" 
            placeholder="Search reports, analytics, metrics..." 
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:block text-right">
            <p className="text-sm font-bold text-foreground leading-tight">{managerData?.name || "Manager User"}</p>
          ...
          <Button onClick={handleLogout} variant="outline" size="sm" className="ml-2 rounded-full border-primary/20 text-primary hover:bg-primary/5 hidden sm:flex gap-2">
